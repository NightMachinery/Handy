...
`external_script_path` is a Linux _paste backend_ (types text via a user
...
runs only when the post-process hotkey fired _and_ AI post-processing is
...
`crate::secure_input::reconcile_fallback(&app)` call landed _inside our
function_. Taking either side naively would have either deleted upstream's
...
`sync_secondary_shortcut_registration()` — so the post-process toggle _and_
...
command wrappers (copied verbatim from the previously _generated_ file), the
two enum aliases, and the six `AppSettings` fields in upstream's new
multi-line format, mirroring the Rust struct's field order. Initially the
regeneration route was blocked: upstream's new ONNX Runtime (`ort` rc12)
prebuilt binary references CoreML symbols (`MLComputePlan`,
`MLOptimizationHints`) that require macOS SDK 14.4+, and Xcode 15.1 ships SDK
14.2, so the app failed to link. After installing Command Line Tools 15.3 and
building with `SDKROOT` pointed at its 14.4 SDK (see `docs/build_mac.md`), the
debug binary was run and its specta export produced a byte-identical
`bindings.ts` — confirming the hand-applied file matches the generated source
of truth exactly.
...
Upstream's new cancellation model _drops_ in-flight pipeline futures when the
...
- Frontend `tsc && vite build` clean; `settingsStore.ts` compiles against the
  ...
  `AppSettings` fields.
- Debug binary builds (with CLT 15.3's SDK 14.4, see above), launches, and its
  specta export regenerates `bindings.ts` byte-identically.
- Not verified: end-to-end dictation with the filter enabled (needs an
  interactive session; the installed Handy instance was running during the
  merge). Worth a manual smoke test on the next dev run.
