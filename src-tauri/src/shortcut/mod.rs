...
use crate::settings::{
    self, get_settings, AutoSubmitKey, ClipboardHandling, CommandFilterOrder, CommandFilterScope,
...
pub fn resume_all_shortcuts(app: &AppHandle) {
    ...
    for (id, binding) in &settings.bindings {
        ...
        if id == "transcribe_with_post_process" && !settings.should_register_secondary_shortcut() {
            continue;
        }
        if let Err(e) = register_shortcut(app, binding.clone()) {
...
fn sync_secondary_shortcut_registration(app: &AppHandle, settings: &settings::AppSettings) {
    ...
    // The secondary hotkey's registration just changed; the SecureInput
    // fallback must mirror the same gate.
    crate::secure_input::reconcile_fallback(app);
}
