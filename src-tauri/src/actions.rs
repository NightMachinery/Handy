...
use crate::settings::{
    get_settings, AppSettings, CommandFilterOrder, OverlayStyle, APPLE_INTELLIGENCE_PROVIDER_ID,
};
use crate::shortcut;
...
/// Run the user-configured stdin/stdout command filter when it applies to the
/// fired hotkey. `None` means the filter is disabled or out of scope.
async fn maybe_apply_command_filter(
    settings: &AppSettings,
    text: &str,
    post_process: bool,
) -> Option<CommandFilterResult> {
    if !settings.command_filter_applies_to_hotkey(post_process) {
        return None;
    }
...
/// Apply the command filter to `final_text` in place. Returns `true` when the
/// filter returned empty output, which cancels the transcription (encoded by
/// the caller as an empty `final_text`, suppressing the paste).
async fn apply_command_filter_stage(
    settings: &AppSettings,
    final_text: &mut String,
    post_process: bool,
    stage: &str,
) -> bool {
    match maybe_apply_command_filter(settings, final_text, post_process).await {
        Some(CommandFilterResult::Applied(filtered)) => {
            *final_text = filtered;
            false
        }
        Some(CommandFilterResult::CancelledEmpty) => {
            info!("Command filter returned empty output; cancelling output");
            true
        }
        Some(CommandFilterResult::Failed(reason)) => {
            error!(
                "Command filter failed {} LLM stage: {}. Falling back to previous text.",
                stage, reason
            );
            false
        }
...
pub(crate) async fn process_transcription_output(
    ...
    let run_filter = settings.command_filter_applies_to_hotkey(post_process);
    let run_filter_before_llm =
        run_filter && settings.command_filter_order == CommandFilterOrder::BeforeLlm;
    let run_filter_after_llm =
        run_filter && settings.command_filter_order == CommandFilterOrder::AfterLlm;
    let mut cancelled_by_filter_empty = false;

    if run_filter_before_llm {
        cancelled_by_filter_empty =
            apply_command_filter_stage(&settings, &mut final_text, post_process, "before").await;
    }

    // The secondary hotkey may be registered for filter-only reasons, so LLM
    // post-processing additionally requires `post_process_enabled`.
    if !cancelled_by_filter_empty && post_process && settings.post_process_enabled {
    ...
    if !cancelled_by_filter_empty && run_filter_after_llm {
        cancelled_by_filter_empty =
            apply_command_filter_stage(&settings, &mut final_text, post_process, "after").await;
    }

    if cancelled_by_filter_empty {
        // An empty `final_text` suppresses the paste (and reads as "nothing to
        // record") on every caller path.
