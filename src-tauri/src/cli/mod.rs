...
//!
//! These are the *application's* flags. The client-facing surface —
//! `handy status`, `handy FILE.wav` — lives in the separate `handy-cli`
//! crate, which forwards anything matching a flag here straight to this
//! binary. Keeping one definition of each flag, in one place, is why the CLI
//! does not model them itself.
...
#[command(
    ...
    after_help = "The `handy` command also speaks to a running instance: \
`handy FILE.wav`, `handy status`.\nSee docs/cli.md."
)]
pub struct CliArgs {
